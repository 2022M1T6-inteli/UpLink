...
export (Resource) var bd_Quiz1 # acesso ao banco de dados
...
func _ready() -> void:
	...
	quiz_shuffle=randomize_array(bd_Quiz1.bd)
	load_quiz()
...
func load_quiz() -> void:
	...
	if index >= bd_Quiz1.bd.size() or current_Dellman_health == 0:
	...
	var options = randomize_array(bd_Quiz1.bd[index].options)
...
func game_over() -> void: #arrumar
	$game_over.show() 
	$game_over/txt_result.text=str(correct, "/", bd_Quiz1.bd.size())

func _on_button_restart_pressed():
	get_tree().reload_current_scene()
	
func _process(delta):
	if current_enemy_health <= 0 or index==20:
		print("entrou no if")
