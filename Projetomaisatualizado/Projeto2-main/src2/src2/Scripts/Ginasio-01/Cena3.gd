...
var playerPosition = Vector2(35, 510)
...
#Com o método add_child(), fazemos com que todos os elementos criados sejam representados em tela
	...
	$Labirinto3/Camera2D.limit_left = 0
	$Labirinto3/Camera2D.limit_bottom = 1019
	$Labirinto3/Camera2D.limit_top = 0
	$Labirinto3/Camera2D.limit_right = 2054
	$Labirinto3/Camera2D.zoom = Vector2(2, 2)
	player.lanterna.texture_scale = 0.4
...
#Cada "if" acima representa a lógica de que se há 2 estrelas no labirinto, 1 já foi coletada e, através do método show(), exibimos as estrelas coletadas com base no número de estrelas restantes
	...
	if Global.openDialogue:
		$Labirinto3/CanvasModulate.visible = false
		add_child(preBalao)
		Global.stop = true
